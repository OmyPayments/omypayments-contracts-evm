...
abstract contract BaseSender is Ownable2StepUpgradeable {
    ...
    modifier onlySender {
        require(senders[msg.sender].exists, CustomError(BaseErrors.SENDER__ONLY_SENDER__ERROR));
        _;
    }

    /// Only owner ow sender modifier
    modifier onlyOwnerOrSender {
        require(senders[msg.sender].exists || msg.sender == owner(), CustomError(BaseErrors.SENDER__ONLY_OWNER_OR_SENDER__ERROR));
        _;
    }
    ...
    function _removeSender(address _sender) internal {
        require(senders[_sender].exists, CustomError(BaseErrors.SENDER__SENDER_NOT_EXISTS__ERROR));
